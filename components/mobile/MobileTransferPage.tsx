...
interface MobileTransferPageProps {
...
export function MobileTransferPage({ 
  ...
  const [positionFilter, setPositionFilter] = useState<string>(outgoingPlayer.position);
  ...
  const teams = useMemo(() => Array.from(
    new Map(allPlayers.map((player) => [player.team, player.teamName || player.team])).entries()
  ).sort((a, b) => a[1].localeCompare(b[1])), [allPlayers]);
  const teamLabel = (player: Player) => player.teamName || player.team;

  const bank = squad.bank;
  const maxBudget = bank + outgoingPlayer.price;

  const handleBankEdit = () => {
    setEditingBank(true);
    setBankInput(maxBudget.toFixed(1));
  };

  const handleBankSave = () => {
    const newMaxBudget = parseFloat(bankInput);
    if (!isNaN(newMaxBudget) && newMaxBudget >= outgoingPlayer.price) {
      const newBank = newMaxBudget - outgoingPlayer.price;
      setBank(newBank);
    }
  ...
  const teamCounts = useMemo(() => {
    ...
    for (const p of all) {
      if (p.id !== outgoingPlayer.id) {
        counts[p.team] = (counts[p.team] || 0) + 1;
      }
    }
    return counts;
  }, [squad, outgoingPlayer]);
  ...
  const filteredPlayers = useMemo(() => {
    const players = allPlayers.filter(p => {
      ...
      if (searchQuery) {
        ...
        const teamMatch = p.team.toLowerCase().includes(query) || 
                         teamLabel(p).toLowerCase().includes(query);
    ...
    return players;
  }, [allPlayers, positionFilter, squadPlayerIds, priceFilter, teamFilter, searchQuery, sortField, sortAsc, maxBudget, teamCounts, weekOffset]);

  const handleSelectPlayer = (player: Player) => {
    removePlayer(outgoingPlayer.id);
    const result = addPlayer(player);
    if (result.ok) {
      toast.success(`Transferred in ${player.name}`);
      onSelectPlayer(player);
    } else {
      addPlayer(outgoingPlayer);
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 flex flex-col">
      ...
      <div className="mx-4 mt-3">
        {editingBank ? (
          <div className="flex items-center justify-center gap-2 bg-emerald-500 rounded-full px-4 py-1.5">
      ...
      <div className="px-4 mt-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-500"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 px-4 mt-3 flex-wrap">
        ...
        >
          ...
          {teams.map(([team, name]) => (
            <option key={team} value={team}>{name}</option>
          ))}
        </select>
      </div>

      <div className="px-4 mt-4">
        <div className="text-xs text-slate-500 mb-2">Outgoing player</div>
        <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
          <div className="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center text-lg">
            👤
          </div>
          <div className="flex-1">
            <div className="font-semibold text-sm">{outgoingPlayer.name}</div>
            <div className="text-xs text-slate-500">
              {teamLabel(outgoingPlayer)} · {outgoingPlayer.position}
            </div>
          </div>
          <div className="text-right">
            <div className="text-sm font-medium">{outgoingPlayer.form?.toFixed(1) || "0.0"}</div>
            <div className="text-xs text-slate-500">Form</div>
          </div>
          <div className="text-right">
            <div className="text-sm font-medium">£{outgoingPlayer.price.toFixed(1)}m</div>
            <div className="text-xs text-slate-500">Price</div>
          </div>
          <div className="text-right">
            <div className="text-sm font-medium">{outgoingPlayer.ownership?.toFixed(1) || "0"}%</div>
      ...
      <div className="flex items-center px-4 mt-4 pb-2 border-b text-xs text-slate-500">
        ...
        <button 
          ...
          className={`w-14 text-center ${sortField === "form" ? "text-violet-400 font-medium" : ""}`}
        >
          Form {sortField === "form" && (sortAsc ? "↑" : "↓")}
        </button>
        <button 
          onClick={() => toggleSort("price")} 
          className={`w-16 text-center ${sortField === "price" ? "text-violet-400 font-medium" : ""}`}
        >
          Price {sortField === "price" && (sortAsc ? "↑" : "↓")}
        </button>
        <button 
          onClick={() => toggleSort("selected")} 
          className={`w-16 text-center ${sortField === "selected" ? "text-violet-400 font-medium" : ""}`}
        >
        ...
        {loading ? (
          ...
          filteredPlayers.map((player: Player) => (
            <div
              ...
              className="w-full flex items-center gap-3 px-4 py-3 border-b border-slate-100 hover:bg-slate-50 active:bg-slate-100 cursor-pointer"
            >
              <div 
                onClick={(e) => { e.stopPropagation(); }}
                className="w-6 h-6 rounded-full bg-violet-500/10 flex items-center justify-center cursor-pointer"
              >
              ...
              <div className="flex-1 text-left">
                ...
                <div className="text-xs text-slate-500">
                  {teamLabel(player)} · {player.position}
                </div>
              </div>
              ...
              <div className="w-16 text-center">
                <span className="text-sm font-medium">£{player.price.toFixed(1)}m</span>
              </div>
